...
"Implemented" significa código **en todas las capas que el requisito necesita** (contrato, backend y pantalla) con al menos una prueba automatizada; "Verified" significa además probado extremo a extremo con cuentas y wallets reales. Mientras falte la pantalla o el contrato, el requisito sigue `In Progress`: el estado por capa está en [traceability.md](traceability.md).
...
| FR-004 | Crear causa                | As a receptor, I want describir mi necesidad con título, descripción y monto objetivo so that los donantes puedan conocerla.                    | High     | Verified  |
...
| FR-011 | Retirar fondos             | As a receptor, I want retirar los fondos recaudados por mi causa verificada so that pueda usarlos para mi necesidad.                            | High     | In Progress  |
| FR-012 | Dashboard de donante       | As a donante, I want ver mis donaciones y las causas disponibles so that llevo control de mi aporte.                                            | Medium   | In Progress         |
| FR-013 | Dashboard de receptor      | As a receptor, I want ver mis causas, su estado y fondos disponibles so that sé cuándo puedo retirar.                                           | Medium   | In Progress         |
...
| NFR-018 | Identidad visual             | La interfaz sigue `docs/design_system.md` (paleta, tipografía, tokens y contraste AA); no se usan colores fuera de los tokens y los estados de causa se ven igual en toda pantalla. | Usability       | Medium   | In Progress  |
| NFR-019 | Idiomas (ES/EN)              | La interfaz se ofrece en español (por defecto) y en inglés, con selector visible y preferencia recordada; el contenido escrito por usuarios (causas) no se traduce. | Usability       | Medium   | Implemented  |
